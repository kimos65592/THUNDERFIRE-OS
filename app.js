...
/* =========================================================
   AI PROVIDER CALL
   Gemini Native REST / Interactions API.

   This adapter intentionally uses the native Gemini endpoint and
   x-goog-api-key authentication. It keeps JARVIS's existing
   Cognitive Core, Tools, Memory and Agent Loop unchanged at the
   orchestration level, while normalizing Gemini function calls back
   into the OpenAI-shaped messages expected by the existing loop.
========================================================= */

const GEMINI_NATIVE_ENDPOINT =
    "https://generativelanguage.googleapis.com/v1beta/interactions";

const GeminiNativeSession = {
    interactionId: null,
    lastFunctionCalls: [],
    active: false
};

function resetGeminiNativeSession() {
    GeminiNativeSession.interactionId = null;
    GeminiNativeSession.lastFunctionCalls = [];
    GeminiNativeSession.active = true;
}

function normalizeGeminiToolDefinitions() {
...
function openAIMessagesToGeminiInput(messages) {
    const input = [];
    ...
    for (const message of Array.isArray(messages) ? messages : []) {
        ...
        /* Tool messages are not replayed here when a native interaction
           id exists; their results are sent as function_result steps. */
    }
...
function extractGeminiFunctionCalls(data) {
    const calls = [];
    const steps = Array.isArray(data?.steps)
        ? data.steps
        : [];
    ...
    for (const step of steps) {
        ...
        let args = step.arguments ?? step.args ?? {};
        ...
        calls.push({
            id: String(step.id || step.call_id || `gemini_fc_${Date.now()}_${calls.length}`),
            name: String(step.name || ""),
            arguments: JSON.stringify(args || {})
        });
    }

    /* Some API revisions expose calls under output instead of steps. */
    ...
    for (const item of output) {
        ...
        const id = String(item.id || item.call_id || `gemini_fc_${Date.now()}_${calls.length}`);
        if (!calls.some(x => x.id === id)) {
            calls.push({
                id,
    ...
    return calls.filter(x => x.name);
}

async function callGeminiNative(messages) {
    const isContinuation =
        Boolean(GeminiNativeSession.interactionId) &&
        Array.isArray(messages) &&
        messages.some(m => m?.role === "tool");

    const body = {
        model: AI_CONFIG.model || "gemini-3.8-flash",
        tools: normalizeGeminiToolDefinitions(),
        store: true
    };

    if (isContinuation) {
        body.previous_interaction_id =
            GeminiNativeSession.interactionId;

        const functionResults = messages
            .filter(m => m?.role === "tool")
            .slice(-GeminiNativeSession.lastFunctionCalls.length)
            .map(m => {
                const call = GeminiNativeSession.lastFunctionCalls
                    .find(c => c.id === m.tool_call_id) ||
                    GeminiNativeSession.lastFunctionCalls[0];

                return {
                    type: "function_result",
                    name: String(m.name || call?.name || ""),
                    call_id: String(m.tool_call_id || call?.id || ""),
                    result: [{
                        type: "text",
        ...
        body.input = functionResults;
    } else {
        body.input = openAIMessagesToGeminiInput(messages);
    }
    ...
    if (!response.ok) {
        ...
        throw new Error(`Gemini Native HTTP ${response.status}: ${text}`);
    }

    const data = await response.json();

    if (data?.id) {
        GeminiNativeSession.interactionId = data.id;
    }

    const toolCalls = extractGeminiFunctionCalls(data);
    GeminiNativeSession.lastFunctionCalls = toolCalls;
...
async function callAI(messages) {
    ...
    if (!AI_CONFIG.apiKey ||
        AI_CONFIG.apiKey === "ضع_مفتاح_Gemini_هنا" ||
        AI_CONFIG.apiKey === "حط_مفتاح_GEMINI_هنا") {
        throw new Error("Gemini API key is missing.");
    }

    return callGeminiNative(messages);
}
