...
function normalizeToolArguments(name, args) {
    ...
    const arrayFields = ["constraints", "preferences"];

    if (name === "goal_create" || name === "goal_update") {
        for (const field of arrayFields) {
            if (typeof safe[field] === "string") {
                safe[field] = safe[field]
                    ...
                    .filter(Boolean);
            } else if (!Array.isArray(safe[field]) && safe[field] != null) {
                safe[field] = [String(safe[field])];
            }
        }

        if (safe.urgency != null) {
            safe.urgency = clamp(safe.urgency, 0, 1);
        }
    }

    if (name === "plan_create" && typeof safe.alternatives === "string") {
        safe.alternatives = [safe.alternatives];
    }

    if (name === "memory_save" && typeof safe.category !== "string" && safe.category != null) {
        safe.category = String(safe.category);
    }
...
async function callGemini(messages) {
    ...
    // Native Gemini Developer API (GenerateContent).
    // This intentionally does NOT use OpenAI-compatible auth or Interactions API.
    const endpoint =
    ...
    const contents = chatMessages.map(message => {
        ...
        // Native Gemini expects only user/model roles.
        let role = message.role === "assistant" ? "model" : "user";

        // Tool results need to be represented as a user-side functionResponse
        // turn when using GenerateContent.
        if (message.role === "tool") {
            role = "user";

            let resultValue = {};
            try {
                resultValue = JSON.parse(message.content || "{}");
            } catch {
                resultValue = { raw: message.content || "" };
            }
    ...
    const body = {
        contents,
        generationConfig: {
            temperature: Number.isFinite(Number(AI_CONFIG.temperature))
                ? Number(AI_CONFIG.temperature)
                : 0.25,
            maxOutputTokens: Number.isFinite(Number(AI_CONFIG.maxTokens))
                ? Number(AI_CONFIG.maxTokens)
                : 1200
        },
        tools: [
            {
...
async function callGeminiWithRetry(messages) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        try {
            ...
            const retryable =
                /Gemini HTTP (429|500|502|503|504)/i.test(text) ||
                /RESOURCE_EXHAUSTED|UNAVAILABLE|DEADLINE/i.test(text);
            ...
            state.agent.retries++;
            EventBus.emit("GEMINI_RETRY", {
                attempt,
                maxAttempts,
                error: text
            });

            const retryMs = Math.min(8000, 700 * Math.pow(2, attempt - 1));
            await sleep(retryMs);
        }
    }
